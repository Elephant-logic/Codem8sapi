(() => {
  ...
  function cmakeMissingFiles(files) {
    ...
    for (const [cmakeName, text] of Object.entries(files)) {
      ...
      while ((match = regex.exec(String(text || '')))) {
        let ref = match[1].replace(/^['"]|['"]$/g, '').replace(/^\.\//, '');
  ...
  function activeProjects() {
    ...
    const keys = [];
    try {
      const active = String(w.localStorage.getItem(ACTIVE_KEY) || '').trim();
    ...
    for (const key of keys) {
      ...
      seen.add(key);
      try {
        const project = JSON.parse(w.localStorage.getItem(key) || 'null');
        if (project?.files && typeof project.files === 'object') projects.push({ key, project, files: cleanProjectFiles(project) });
      } catch {}
    }
    return projects;
  }
  function bestBuildFiles(item) {
    ...
    const candidates = [{ source: 'saved app', files: saved, project: item?.project }];
    for (const entry of activeProjects()) candidates.push({ source: 'active editor', files: entry.files, project: entry.project });
    const scored = candidates.filter(candidate => hasJuceProject(candidate.files)).map(candidate => ({
      ...candidate,
      missing: cmakeMissingFiles(candidate.files),
      score: Object.keys(candidate.files).length * 10 - cmakeMissingFiles(candidate.files).length * 1000
    })).sort((a, b) => b.score - a.score);
    return scored[0] || { source: 'saved app', files: saved, project: item?.project, missing: [], score: 0 };
  }

  function settings(item, selection) {
    return new Promise(resolve => {
      ...
      panel.querySelector('[data-build]').onclick = () => {
        ...
        if (custom && missing.length) return void (error.textContent = `Project is incomplete. Missing: ${missing.slice(0, 4).join(', ')}. Open or re-import the complete project before building.`);
  ...
  function progress(name) {
    ...
    document.body.appendChild(panel); panel.querySelector('[data-close]').onclick = () => panel.remove();
  ...
  async function build(id) {
    ...
    const selection = bestBuildFiles(item);
    const chosen = await settings(item, selection); if (!chosen) return;
    ...
    try {
      ...
      while (Date.now() - begin < 35 * 60 * 1000) {
        ...
        const status = await requestJson(`/api/vst-builds/${encodeURIComponent(started.id)}`); addWorkflowLink(ui, status.runUrl);
