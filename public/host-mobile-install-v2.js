(() => {
  ...
  const badge=document.getElementById('codem8s-version');if(badge)badge.textContent='Codem8s 10.8.9';
  async function cleanOldWorker(){
    try{
      ...
      if('caches'in window){
        ...
        await Promise.all(keys.filter(key=>key.startsWith('codem8s-mobile-')&&!['codem8s-mobile-meta-v2','codem8s-mobile-meta-v3'].includes(key)).map(key=>caches.delete(key)));
      }
    }catch{}
  }
  cleanOldWorker();
