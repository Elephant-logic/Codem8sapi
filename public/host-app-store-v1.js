(() => {
  ...
  if (badge) badge.textContent = 'Codem8s 10.8.1';
  ...
  function save(name) {
    ...
    render();
    status(`Saved “${title}” on this device.`);
  }
  function open(id) {
  ...
  function render() {
    ...
    grid.innerHTML = list.length ? list.map(x => `<article style="border:1px solid #29425f;background:#091626;border-radius:14px;padding:12px;display:grid;gap:9px"><div><strong>${esc(x.name)}</strong><div style="color:#8fa4c1;font-size:11px">${esc(x.type)} · ${x.fileCount} files</div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:7px"><button class="toolbtn" data-open="${x.id}">Open</button><button class="toolbtn" data-copy="${x.id}">Duplicate</button><button class="toolbtn" data-export="${x.id}">Export</button><button class="toolbtn" data-remove="${x.id}">Delete</button></div></article>`).join('') : '<div class="empty">No saved apps yet. Enter a name and tap Save current app.</div>';
  }
