(() => {
  ...
  if (badge) badge.textContent = 'Codem8s 10.8.0';
  const STORE = 'codem8s_app_store_v1';
  const KEYS = ['codem8s_project_v4','codem8s_project_v7','codem8s_project_v8'];
  ...
  function save(name, quiet = false) {
  ...
  function status(text) { const node = appDoc()?.querySelector('#appsStoreStatus'); if (node) node.textContent = text; }
  function render() {
    ...
    grid.innerHTML = list.length ? list.map(x => `<article style="border:1px solid #29425f;background:#091626;border-radius:14px;padding:12px;display:grid;gap:9px"><div><strong>${esc(x.name)}</strong><div style="color:#8fa4c1;font-size:11px">${esc(x.type)} · ${x.fileCount} files</div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:7px"><button class="toolbtn" data-open="${x.id}">Open</button><button class="toolbtn" data-copy="${x.id}">Duplicate</button><button class="toolbtn" data-export="${x.id}">Export</button><button class="toolbtn" data-remove="${x.id}">Delete</button></div></article>`).join('') : '<div class="empty">No saved apps yet.</div>';
  }
  function wire() {
    ...
    pane.onclick = e => { const b = e.target.closest('button'); if (!b) return; if (b.dataset.open) open(b.dataset.open); if (b.dataset.copy) duplicate(b.dataset.copy); if (b.dataset.export) exportApp(b.dataset.export); if (b.dataset.remove) remove(b.dataset.remove); };
    render();
  }
